...
function _teamCard(t) {
  ...
  const dotBg = colorBg(t.color, t.color2, t.pattern);
  html += '<div class="ac-name"><span class="ac-dot" style="background:' + escHTML(dotBg) + '"></span>' + escHTML(t.name || 'Sans nom') + '</div>';
  ...
  html += '<div class="progress-track" style="margin-top:8px"><div class="progress-fill" style="transform:scaleX(' + (pct / 100) + ');background:' + escHTML(dotBg) + '"></div></div>';
...
function renderTabEquipes(body, teams) {
  ...
  teams.forEach(t => {
    const captain = typeof t.captain === 'object' ? (t.captain && t.captain.name || 'N/A') : (t.captain || '—');
    const listDotBg = colorBg(t.color, t.color2, t.pattern);
    html += '<div class="team-list-item">';
    html += '<div class="team-list-name"><span class="ac-dot" style="background:' + escHTML(listDotBg) + '"></span>' + escHTML(t.name || 'Sans nom') + '</div>';
...
async function renderColorGrid() {
  ...
  try {
    ...
    const usedIds = new Set((data.teams || []).map(t => t.id));
    el.innerHTML = '';
    (data.colors || []).forEach(c => {
      const circle = document.createElement('div');
      circle.className = 'color-circle';
      if (usedIds.has(c.id)) circle.classList.add('used');
