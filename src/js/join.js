...
async function loadTeams() {
  ...
  try {
    ...
    data.teams.forEach(t => {
      ...
      btn.innerHTML = `<div class="dot" style="background:${colorBg(t.color, t.color2, t.pattern)}"></div><div class="tname">${escHTML(t.name)}</div>${t.taken ? '<span class="taken-badge">Prise</span>' : ''}`;
